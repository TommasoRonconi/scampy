...
double sico::cosmology::int_phi_Lapi17 ( const double & Lmin, const double & Lmax,
					  const double & zz, const std::vector< double > & param ) {
  ...
  const double xmin = std::pow( 10, - 0.4 * ( Lmin - 4.83 ) - 9.8 ) / sfrc;
  const double xmax = std::pow( 10, - 0.4 * ( Lmax - 4.83 ) - 9.8 ) / sfrc;

  return - 2.5 * Norm * ( gsl_sf_gamma_inc( 1 - alpha, xmax ) - gsl_sf_gamma_inc( 1 - alpha, xmin ) );  
  
}

//==============================================================================================

double sico::cosmology::int_phi_Lapi17_uv ( const double & Lmin, const double & Lmax, const double & zz ) {

  const std::vector< double > param = { -1.96, -1.60, 4.22, -5.23, //philog
					1.11, 2.85, -6.18, 4.44,   //alphap
					0.01, 2.85, 0.43, -1.70 }; //sfrlog

  return int_phi_Lapi17 ( Lmin, Lmax, zz, param );

}

//==============================================================================================

double sico::cosmology::int_phi_Lapi17_uvir ( const double & Lmin, const double & Lmax, const double & zz ) {
